...
def signal_chunker(source_dir, target_dir, chunk_size=10000):
    """
    Splits IQ signal files into smaller chunks of specified size.
    """
    os.makedirs(target_dir, exist_ok=True)
    for filename in os.listdir(source_dir):
        if filename.endswith('.csv'):
            file_path = os.path.join(source_dir, filename)
            df = pd.read_csv(file_path, skiprows = 10, chunksize=chunk_size, header=None)
            for i, chunk in enumerate(df):
                chunk_file_path = os.path.join(target_dir, f"{filename}_chunk_{i}.csv")
